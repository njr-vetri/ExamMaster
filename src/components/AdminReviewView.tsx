  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fadeIn">
      ...
      <div className="bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 shadow-2xs">
        ...
        <div className="flex items-center gap-2.5 w-full md:w-auto">
          <button
            type="button"
            onClick={onCancel}
            className="px-4 py-2 text-xs font-bold text-slate-500 hover:text-slate-805 rounded-lg transition-colors select-none"
          >
            Discard All
          </button>
          ...
          <button
            ...
            className="flex-1 md:flex-none px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:pointer-events-none text-white rounded-xl text-xs font-bold shadow-sm hover:shadow-md transition-all inline-flex items-center justify-center gap-1.5"
            id="admin-publish-exam"
          >
            <Play className="h-3.5 w-3.5 fill-current" />
            Publish & Start Test ({approvedCount} Qs)
          </button>
        </div>
      </div>
      ...
      <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-4 flex items-center justify-between text-xs">
        ...
        <span className="text-slate-500">Select any question below to expand & edit</span>
      </div>
                ...
                <div className="p-4 sm:p-5 flex items-center justify-between gap-4 cursor-pointer select-none">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      ...
                      className="h-7 w-7 rounded-lg bg-slate-50 hover:bg-red-50 text-slate-400 hover:text-red-600 flex items-center justify-center transition-colors"
                    ...
                    <button
                      ...
                      className="h-7 w-7 text-slate-400 flex items-center justify-center"
                    >
                    ...
                    {isEditing ? (
                      ...
                      <div className="space-y-4">
                        <div className="space-y-1.5">
                          <label className="text-3xs font-bold text-slate-400 uppercase">Question Text</label>
                          <textarea
                            value={editText}
                            onChange={(e) => setEditText(e.target.value)}
                            rows={3}
                            className="w-full text-slate-800 text-sm p-3 bg-slate-50 border border-slate-205 rounded-xl focus:border-indigo-500 outline-hidden"
                          />
                        </div>

                        {/* Options Editor */}
                        <div className="space-y-2">
                          <label className="text-3xs font-bold text-slate-400 uppercase block">MCQ Options (Select the correct radio)</label>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {editOptions.map((opt, oIdx) => (
                              ...
                              >
                                ...
                                />
                                <span className="text-xs font-semibold text-slate-400 min-w-[15px]">
                                ...
                                <input
                                  ...
                                  className="bg-transparent border-b border-dashed border-slate-300 focus:border-slate-800 outline-hidden py-0.5 text-xs text-slate-700 flex-1"
                                />
                              </div>
                            ))}
                          </div>
                        ...
                        {/* Explanation Editor */}
                        <div className="space-y-1.5">
                          <label className="text-3xs font-bold text-slate-400 uppercase">Explanation / Rationalization</label>
                          <textarea
                            value={editExplanation}
                            onChange={(e) => setEditExplanation(e.target.value)}
                            rows={2}
                            className="w-full text-slate-700 text-xs p-3 bg-slate-50 border border-slate-205 rounded-xl focus:border-indigo-500 outline-hidden"
                          />
                        </div>
                      ...
                      <div className="space-y-4">
                        {/* Options List */}
                        <div className="space-y-2">
                          <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Answer Options</span>
                              ...
                              return (
                                ...
                                >
                                  <span className={`h-5 w-5 rounded-md flex items-center justify-center text-[10px] font-bold ${
                                    isCorrect ? 'bg-emerald-600 text-white' : 'bg-slate-200/80 text-slate-500'
                                  }`}>
                                    {String.fromCharCode(65 + oIdx)}
                                  </span>
                                  <span>{opt}</span>
                        ...
                        <div className="bg-slate-50 rounded-xl p-3 border border-slate-100 space-y-1">
                          <span className="text-[9px] font-bold uppercase tracking-wider text-slate-400 block">AI Rational explanation</span>
                        ...
                        <div className="flex justify-end pt-1">
                          <button
                            ...
                            className="px-3 py-1.5 hover:bg-slate-100 text-slate-500 hover:text-slate-800 rounded-lg text-xs font-semibold inline-flex items-center gap-1 transition-colors"
                          >
